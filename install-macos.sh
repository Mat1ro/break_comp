...
set -euo pipefail
# Включая stdout дочерних pip/venv; ошибки остаются в stderr.
exec > /dev/null

if [[ "${1:-}" == "--help" ]]; then
    echo "Использование: bash install-macos.sh" >&2
    echo "Скачивает проект, устанавливает зависимости и включает автозапуск. Нужен Python 3.9+." >&2
    exit 0
fi
if [[ $# -gt 0 ]]; then
...
for name in main.py audio_player.py brightness_controller.py stop_hotkey.py wallpaper_controller.py assets/farts-8.mp3 assets/poop.webp assets/poop.png autostart.py requirements.txt README.md; do
